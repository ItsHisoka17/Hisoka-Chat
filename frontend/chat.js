...
function handleUserName() {
  ...
  */
    ...
    $('#username').submit(function(e) {
      ...
      window.location.replace(`https://Hisoka-Chat.hisoka17.repl.co?u=${name}&pw=${pw}`);
    })
  }
};
    ...
    document.getElementById('img').addEventListener('change', (e) => {
      ...
      setTimeout(() => {
        ...
        reader.onload = function(r) {
          ...
          socket.emit('message', {
            user: socket.username, message: `<img src="${r.target.result}" onclick="window.open(${r.target.result})" style="width: 25%; height: 20%; border-radius: 5%;">`
          })
        };
...
function userCount() {
  ...
  socket.on('usercount', (data) => {
    $('.users').html(`Online User${data.length > 1 ? 's' : ''}: ${data.length}<br></br>${data.join('<br></br>')}`)
  })
}
